...
def test_compile_hello_agent(hello_world_agent_dir: Path) -> None:
    ...
    assert isinstance(compiled, CompiledAgentIR)
    assert compiled.schema_version == IR_SCHEMA_VERSION
...
def test_compile_mcp_example(mcp_agent_dir: Path) -> None:
    ...
    compiled = compile_agent_ir(mcp_agent_dir)
    ...
    assert any(rule.decision == "deny" for rule in compiled.policies)
    ...
    delegate_tool = next(tool for tool in compiled.tools if tool.name == "delegate_to_time_checker")
    assert delegate_tool.metadata.get("subagent") == "time-checker"
    ...
    assert compiled.capabilities.enable_subagents is True
...
def test_compile_production_does_not_require_deployment(mcp_agent_dir: Path) -> None:
    ...
    compiled = compile_agent_ir(mcp_agent_dir, production=True)
...
def test_compile_locked_mode_disables_dangerous_tools(hello_world_agent_dir: Path) -> None:
    ...
    assert "run_command" in compiled.capabilities.disabled_tools
    assert "create_file" in compiled.capabilities.disabled_tools
